...
import dimas.org.apache.maven.model.path.DefaultPathTranslator
...
class UpdateTask extends DefaultTask {
    ...
    void run() {
            ...
            Object apply(Map<ModuleComponentIdentifier, Set<File>> repositoryFiles) {
                repositoryFiles.each { id, files ->
                    ...
                    files.each { File file ->
                        ...
                        boolean isTargetMavenLocal = directory.absolutePath.contains(".m2")

                        // TODO: allow when source is mavenLocal() and target isn't mavenLocal()
                        boolean allow1 = isSourceMavenLocal && !isTargetMavenLocal
                        // TODO: allow when source isn't mavenLocal() and target is mavenLocal()
                        boolean allow2 = !isSourceMavenLocal && isTargetMavenLocal
        ...
        println("Log saved ${getLogFile().absolutePath}")
    ...
    private Set<Configuration> getConfigurations() {
        ...
        if (this.getConfigurationNames()) {
            ...
            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Trying to resolve the following project configurations: '${configurationNames.join(",")}'")
            ...
            configurationNames.each { name ->
                try {
                    ...
                    if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.warn("Unable to resolve project configuration with name '${name}'")
                }
            }
        } else {
            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("No project configurations specified, defaulting to all configurations")
        ...
        if (this.getIncludeBuildscriptDependencies()) {
            if (this.getBuildscriptConfigurationNames()) {
                ...
                if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Trying to resolve the following buildscript configurations: '${configurationNames.join(",")}'")
                ...
                configurationNames.each { name ->
                    try {
                        ...
                        if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.warn("Unable to resolve buildscript configuration with name '${name}'")
                    }
                }
            } else {
                if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("No buildscript configurations specified, defaulting to all configurations")
                configurations.addAll(project.buildscript.configurations)
            }
        } else {
            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Skipping buildscript configurations")
        }

        if (!configurations) {
            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.warn('No configurations found. There are no dependencies to resolve.')
        }
    ...
    private Map<ModuleComponentIdentifier, Set<File>> collectRepositoryFiles(Set<Configuration> configurations) {
            ...
            for (dependency in configuration.allDependencies) {
                if (dependency instanceof ExternalModuleDependency) {
                    ...
                    cfg.resolvedConfiguration.resolvedArtifacts.forEach({ artifact ->
                        ...
                        if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Adding artifact for dimas.java.component'{}' (location '{}')", componentId, artifact.file)
                ...
                if (this.getIncludeSources()) {
                    ...
                    if (!sources?.empty) {
                        sources*.file.each { File source ->
                            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Adding sources for dimas.java.component'{}' (location '{}')", component.id, source)
                ...
                if (this.getIncludeJavadocs()) {
                    ...
                    if (!javadocs?.empty) {
                        javadocs*.file.each { File javadoc ->
                            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Adding javadocs for dimas.java.component'{}' (location '{}')", component.id, javadoc)
        ...
        // collect maven poms (for immediate dimas.java.component ids and parents)
    ...
    // adds pom artifacts and their parents for the givens dimas.java.component ids
    private void collectPoms(Set<ComponentIdentifier> componentIds, Map<ComponentIdentifier, Set<File>> repositoryFiles) {
        if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Collecting pom files")
        ...
        for (component in mavenArtifacts.resolvedComponents) {
            ...
            if (pomArtifact instanceof UnresolvedArtifactResult) {
                if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.error("Resolver was unable to resolve artifact '{}'", pomArtifact.id, pomArtifact.getFailure())
                continue
            }
            ...
            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Adding pom for dimas.java.component'{}' (location '{}')", component.id, pomFile)
    ...
    private Model resolvePom(PomDependencyModelResolver pomModelResolver, File pomFile) {
        ...
        try {
            ...
            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.error("${e.getMessage()}: ${e.problems}")
        }
    }
    ...
    protected void logModelProblems(ModelProblem problem) {
        ...
        switch (problem.severity) {
            case ModelProblem.Severity.WARNING:
                if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.info(message, problem.exception)
                break

            case ModelProblem.Severity.ERROR:
            case ModelProblem.Severity.FATAL:
                if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.error(message, problem.exception)
                break
        }
    }

    private void collectIvyXmls(Set<ComponentIdentifier> componentIds, Map<ComponentIdentifier, Set<File>> repositoryFiles) {
        if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Collecting ivy xml files")
        ...
        for (component in ivyArtifacts.resolvedComponents) {
            ...
            if (ivyXmlArtifact instanceof UnresolvedArtifactResult) {
                if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.error("Resolver was unable to resolve artifact '{}'", ivyXmlArtifact.id, ivyXmlArtifact.getFailure())
                continue
            }

            def ivyXml = ivyXmlArtifact.file as File
            if (getExtension().debug) com.dimaslanjaka.gradle.helper.Logger.trace("Adding ivy artifact for dimas.java.component'{}' (location '{}')", component.id, ivyXml)
    ...
    private def withRepositoryFiles(Closure<Map<ModuleComponentIdentifier, Set<File>>> callback) {
        ...
        Extension extension = project.extensions.getByName(Plugin.EXTENSION_NAME) as Extension
