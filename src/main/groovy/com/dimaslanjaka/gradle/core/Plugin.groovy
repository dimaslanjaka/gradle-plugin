...
import com.dimaslanjaka.gradle.versioning.Semver
...
class Plugin implements org.gradle.api.Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        def repositoryResolver = new Resolver(project)
        ...
        if (!project.hasProperty('versioning')) {
            def semver = new Semver(project.version)
            project.versioning {
                major semver.major
                minor semver.minor
                patch semver.patch
                preRelease semver.pre
            }
        }
        ...
        //project.tasks.findByName('clean').finalizedBy('updateOfflineRepository')
