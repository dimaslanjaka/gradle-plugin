...
class GrabVer {
    ...
    protected boolean autoVersionFirstRun = true
    def configurationExist = false
    ...
    void apply(Project project) {
        ...
        project.gradle.buildFinished() { BuildResult result ->
            configurationExist = project.getConfigurations().find { it.name == 'versioning' } && project.hasProperty('versioning')
            println("versioning " + (configurationExist ? styler(GREEN, "exist") : styler(RED, "not found")))
            if (configurationExist) {
                if (firstRun || !succeededTasks.isEmpty()) {
                    ...
                    // TODO: Save new versioning
                    saveFile()
                } else if (result.failure != null) {
                    println(styler(RED, project.name.toUpperCase() + ' - ' + result.failure.getLocalizedMessage()))
                }
            }
            // TODO: Refresh Auto Version Code Each Build Finished
            setAutoVersion(project)
        }
        // TODO: Set Fixed Version Code
        setAutoVersion(project)
    }

    void setAutoVersion(Project project) {
        Auto.refresh()

        // Set Auto Project Version Code Build Based On Current Date
        project.ext["AutoVersionCode"] = Auto.VersionCode
        assert project.AutoVersionCode == Auto.VersionCode
        assert project.ext["AutoVersionCode"] == Auto.VersionCode
        if (autoVersionFirstRun) {
            project.ext["FixedVersionCode"] = Auto.FixedVersionCode
            assert project.FixedVersionCode == Auto.FixedVersionCode
            assert project.ext["FixedVersionCode"] == Auto.FixedVersionCode
            autoVersionFirstRun = false
        }
    }
    ...
    static String getVersionPropertiesFile(Project project1) {
        return project1.projectDir.absolutePath + File.separator + VERSIONING_FILENAME
    }

    private File getFile(boolean silent) {
        String filename = getVersionPropertiesFile(this.project)
        def test = new Properties(new File(filename), true)
        if (!test.contains("MAJOR")) test.set("MAJOR", "1")
        if (!test.contains("PATCH")) test.set("PATCH", "0")
        if (!test.contains("MINOR")) test.set("MINOR", "0")
        if (!test.contains("BUILD")) test.set("BUILD", "0")
        if (!test.contains("CODE")) test.set("CODE", "1")
        if (!test.contains("PRE_RELEASE")) test.set("PRE_RELEASE", "SNAPSHOT")
    ...
    private static boolean shouldSave(List<String> runTasks, String project, String saveOn) {
        if (runTasks.empty) return false
